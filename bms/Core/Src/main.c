...
fdcan_hardware_init();
  ...
  defaultTaskHandle = osThreadNew(bms_safety_task, NULL, &defaultTask_attributes);
...
void HAL_FDCAN_RxFifo1Callback(FDCAN_HandleTypeDef *hfdcan,
  ...
  if ((RxFifo0ITs & FDCAN_IT_RX_FIFO1_NEW_MESSAGE) != RESET) {
    ...
    if (HAL_FDCAN_GetRxMessage(hfdcan, FDCAN_RX_FIFO1, &rxHeader, msg.data) ==
        HAL_OK) {
      msg.id = rxHeader.Identifier;
      // this can2
      if (hfdcan->Instance == FDCAN1) {
...
/**
  ...
  * @retval None
  */
/* USER CODE END Header_bms_safety_task */
void bms_safety_task(void *argument)
{
  /* USER CODE BEGIN 5 */
  /* Infinite loop */
