...
static HAL_StatusTypeDef forward_can2_to_fdvcan(const can2_msg_t *msg) {
  ...
  case PLACEHOLDER_CURRENT_SENSOR_CAN_ID:
    header.Identifier =
        can_id_build(CAN_PRIORITY_P1, NODE_RASPI, BMS_CURRENT_SENSOR, NODE_BMS);
    break;
  case IMD_CAN_ID:
    header.Identifier =
        can_id_build(CAN_PRIORITY_P1, NODE_RASPI, BMS_IMD_REQUEST, NODE_BMS);
    break;
  case IMD_CAN_ID_RESPONSE:
    header.Identifier =
        can_id_build(CAN_PRIORITY_P1, NODE_RASPI, BMS_IMD_RESPONSE, NODE_BMS);
    break;
  case IMD_CAN_ID_GENERAL:
    header.Identifier =
        can_id_build(CAN_PRIORITY_P1, NODE_RASPI, BMS_IMD_GENERAL, NODE_BMS);
    break;
  case IMD_CAN_ID_ISO_DETAIL:
    header.Identifier =
        can_id_build(CAN_PRIORITY_P1, NODE_RASPI, BMS_IMD_ISO_DETAIL, NODE_BMS);
    break;
  case IMD_CAN_ID_VOLTAGE:
    header.Identifier =
        can_id_build(CAN_PRIORITY_P1, NODE_RASPI, BMS_IMD_VOLTAGE, NODE_BMS);
    break;
  case IMD_CAN_ID_IT_SYSTEM:
    header.Identifier =
        can_id_build(CAN_PRIORITY_P1, NODE_RASPI, BMS_IMD_IT_SYSTEM, NODE_BMS);
    break;
  default:
