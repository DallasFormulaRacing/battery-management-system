...
/**
 * @brief ACTUAL application-level RX handler. Runs when RX FIFO0 gets a new frame.
 *        This is the function Core dispatches to via the registered function pointer.
 */
static void bms_can_rx_handler(const FDCAN_RxHeaderTypeDef *hdr,
                               const uint8_t *data,
                               void *ctx)
                                           ...
                                           const uint8_t *tx)
{
    ...
    (void)fdcan_send(can_id, (uint8_t *)tx, FDCAN_DLC_BYTES_48);
}
                                  ...
                                  cell_voltage_type_t voltage_type)
{
    ...
    for (uint8_t ic = 0; ic < (uint8_t)IC_COUNT_CHAIN; ic++) {
        ...
        for (uint8_t current_cell = 0;
             current_cell < (uint8_t)ADBMS_NUM_CELLS_PER_IC;
             current_cell++)
        {
            int16_t voltage;
